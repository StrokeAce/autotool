...
def image_pyramid():#预处理，图像金字塔
    image_find =cv2.imread("./data/2.png")
    image_big = cv2.imread("./data/1.png")
    for i in range(6):
        image_find = cv2.pyrDown(image_find)
        #cv2.imshow('1',image_find)
        #print(image_find.shape)
    cv2.imwrite('./data/1_temp.png', image_find)
    for i in range(3):
        image_big = cv2.pyrDown(image_big)
        #cv2.imshow('1',image_big)
        #print(image_find.shape)
    cv2.imwrite('./data/2_temp.png', image_big)

def image_contrast():#再次读取图片进行操作
    image_find = Image.open("./data/1_temp.png")
    image_big = Image.open("./data/2_temp.png")
